...
describe('HighlightGridComponent', () => {
  it('rend une carte par élément mis en avant', async () => {
    const fixture = await mount(HighlightGridComponent, {
      inputs: { items: highlights },
    });

    expect(
      (fixture.nativeElement as HTMLElement).querySelectorAll('app-icon')
        .length,
    ).toBe(highlights.length);
  });
});
