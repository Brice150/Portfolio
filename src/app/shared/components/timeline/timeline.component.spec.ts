...
describe('TimelineComponent', () => {
  it('rend une entrée par jalon', async () => {
    ...
    const fixture = await mount(TimelineComponent, { inputs: { milestones: items } });

    expect((fixture.nativeElement as HTMLElement).querySelectorAll('li.entry').length).toBe(items.length);
  });

  it('associe une icône et un libellé à chaque nature de jalon', async () => {
    const fixture = await mount(TimelineComponent, { inputs: { milestones: [] } });
